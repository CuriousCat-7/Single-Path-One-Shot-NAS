...
def get_model_complexity_info(model, input_res,
    ...
    if type(input_res) is not tuple:
        tuple(input_res)
...
def params_to_string(params_num, units=None, precision=2):
    if units is None:
        if params_num // 10 ** 9 > 0:
            return str(round(params_num / 10 ** 9, 2)) + ' G'
...
def empty_flops_counter_hook(module, input, output):
    #print(f"skip flops counting for {module}; use empty_flops_counter")
    module.__flops__ += 0
...
if __name__ == '__main__':
    ...
    from model import SinglePath_Network
    ...
    flops, params = get_model_complexity_info(
            model,
            (3,32,32) if args.dataset=="cifar10" else (3,224,224))
    logger.info("flops {}, params {}", flops, params)
